...
class ItemKNN(Algorithm):
    ...
    def fit(self, X: scipy.sparse.csr_matrix) -> 'ItemKNN':
        ...
        if self.k:
            top_k_per_row = np.argpartition(item_cosine_similarities_, -self.k,
                                            axis=1)[:, -self.k:]
            values = np.take_along_axis(item_cosine_similarities_,
                                        top_k_per_row, axis=1)
        ...
        if self.normalize:
            ...
            nanLocations = np.isnan(item_cosine_similarities_)
            item_cosine_similarities_[nanLocations] = 0
    ...
    def predict(self, histories: scipy.sparse.csr_matrix) -> np.ndarray:
        ...
        # print(self.similarity_matrix_.shape)
