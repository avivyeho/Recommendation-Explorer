...
def df_to_csr(df: pd.DataFrame):
    ...
    return scipy.sparse.csr_matrix((data, (df[USER_ID], df[ITEM_ID])),
                                   dtype=np.int8)
...
def predictions_to_recommendations(predictions: np.ndarray,
                                   top_k: int) -> np.ndarray:
    """ Takes a matrix of user-item scores and returns a ranked list of the top_k items per user. """
    if top_k >= predictions.size:
        top_k = predictions.size
    recommendations = np.argpartition(predictions, -1 - np.arange(top_k),
                                      axis=1)[:, -top_k:][:, ::-1]
